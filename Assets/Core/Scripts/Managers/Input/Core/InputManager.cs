...
{
    [CreateAssetMenu(menuName = "Terraknown/Managers/Input", fileName = "InputManager")]
    public class InputManager : Manager<InputManager>
    {
        ...
        [SerializeField] [NotNull] private InputContextGroup m_DefaultContext;
        ...
        public static void PopContext(InputContextGroup context)
        {
            ...
            var index = m_Contexts.IndexOf(context);
            ...
            if (index != -1)
            {
                ...
                if (m_ActiveContext == context)
                {
                    var contextToEnable = m_Contexts.Count == 0
                        ? Instance.m_DefaultContext
                        : m_Contexts[m_Contexts.Count - 1];
        ...
        public static bool HasEscapeCallbacks =>
            s_EscapeCallbacks.Count > 0 || s_LastEscapeCallbackRemoveFrame == Time.frameCount;

        [SerializeField] private InputActionReference m_EscapeInput;
        ...
        public static void PushEscapeCallback(UnityAction action)
        {
            ...
            var index = s_EscapeCallbacks.IndexOf(action);
        ...
        private void RaiseEscapeCallback(InputAction.CallbackContext callbackContext)
        {
            if (s_EscapeCallbacks.Count == 0)
                return;

            var lastCallbackIndex = s_EscapeCallbacks.Count - 1;
