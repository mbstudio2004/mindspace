...
namespace Nocci.Utils
{
    public static class PhysicsUtils
    {
        ...
        {
            var raySpreadVector = transform.TransformVector(new Vector3(Random.Range(-randomSpread, randomSpread),
                Random.Range(-randomSpread, randomSpread), 0f));
            var rayDirection = Quaternion.Euler(raySpreadVector) * transform.forward;
        ...
        public static int OverlapBoxNonAlloc(Vector3 center, Vector3 extents, Quaternion orientation,
            out Collider[] colliders, LayerMask layers,
            QueryTriggerInteraction triggerInteraction = QueryTriggerInteraction.Ignore)
        {
            var size = Physics.OverlapBoxNonAlloc(center, extents, s_OverlappedColliders, orientation, layers,
                triggerInteraction);
        ...
        public static int OverlapSphereNonAlloc(Vector3 position, float radius, out Collider[] colliders,
            LayerMask layers, QueryTriggerInteraction triggerInteraction = QueryTriggerInteraction.Ignore)
        {
            var size = Physics.OverlapSphereNonAlloc(position, radius, s_OverlappedColliders, layers,
                triggerInteraction);
