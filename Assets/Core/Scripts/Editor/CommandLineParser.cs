...
namespace Jenkins {
    ...
    public class CmdLine : MonoBehaviour 
    {
        ...
        public static void parseCommandLineArgs() {
            string programName = "E:/BUILDS/Terraknown"; // hardcoded here, but can also be passed in with -appname argument

            string[] args = System.Environment.GetCommandLineArgs();
            int i = 0;
            string path = "E:/BUILDS/Terraknown";
            foreach (string arg in args)
            {
                ...
                if (i > 0 && arg == "-buildWindows64Player")
                {
                    path = args[i+1];
                }
                i++;            
            }
            BuildReport report = BuildPipeline.BuildPlayer(EnabledLevels(), path, BuildTarget.StandaloneWindows, buildPlayerOptions.options);
